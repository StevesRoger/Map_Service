...
import { Search, Filter, Calendar as CalendarIcon, Download, ChevronDown, Eye, X, AlertCircle, CheckCircle2, Clock, Copy, ChevronUp } from 'lucide-react';
...
function CopyButton({ text, successMessage, errorMessage, size = 'md' }: CopyButtonProps) {
...
function StatusBadge({ statusCode }: StatusBadgeProps) {
  ...
  return (
    <div className={`inline-flex h-7 rounded-lg border items-center justify-center px-3 ${style.bg} ${style.border}`}>
      <span className={`${style.text} text-sm tracking-[-0.1504px] font-en`}>{statusCode}</span>
    </div>
  );
}
...
function MobileRequestCard({ request, isExpanded, onToggleExpand, apiKeyName, onCopy }: MobileRequestCardProps) {
      ...
      >
        <div className="flex-1 min-w-0 space-y-2">
          <div className="flex items-center gap-2 flex-wrap">
            <div className={`inline-flex items-center justify-center px-2.5 py-1 rounded-lg border ${statusStyle.bg} ${statusStyle.border}`}>
              <span className={`${statusStyle.text} text-sm font-en`}>{request.statusCode}</span>
            </div>
            <div className="inline-flex items-center justify-center px-2.5 py-1 rounded-lg border border-zinc-700/50 bg-zinc-800/30">
              <span className="text-zinc-300 text-sm font-en">{request.method}</span>
            </div>
          </div>
          <p className="text-zinc-300 text-sm truncate font-en">{request.endpoint}</p>
        ...
        {isExpanded ? (
          <ChevronUp className="w-5 h-5 text-zinc-500 shrink-0" aria-hidden="true" />
        ) : (
          <ChevronDown className="w-5 h-5 text-zinc-500 shrink-0" aria-hidden="true" />
        )}
      </button>
        ...
        <div className="px-4 pb-4 space-y-3 border-t border-zinc-800/80">
          <div className="pt-3 grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <p className="text-xs text-zinc-500 uppercase tracking-wider">Response Time</p>
              <p className="text-sm text-rose-400 font-en">{request.responseTime}ms</p>
            </div>
            <div className="space-y-1">
              <p className="text-xs text-zinc-500 uppercase tracking-wider">Cost</p>
              <p className="text-sm text-rose-400 font-en">- ${request.cost.toFixed(4)}</p>
            </div>
            <div className="space-y-1">
              <p className="text-xs text-zinc-500 uppercase tracking-wider">IP Address</p>
              <p className="text-sm text-zinc-400 font-en">{request.ipAddress}</p>
            </div>
            <div className="space-y-1">
              <p className="text-xs text-zinc-500 uppercase tracking-wider">API Key</p>
              <div className="flex items-center gap-2">
                <p className="text-sm text-zinc-400 truncate font-en">{apiKeyName}</p>
                <button
...
function DesktopRequestRow({ request, apiKeyName, onCopy }: DesktopRequestRowProps) {
  const statusStyle = getStatusBadgeStyle(request.statusCode);
    ...
    <tr className="border-b border-zinc-800 hover:bg-zinc-900/50 transition-colors duration-150">
      ...
      <td className="h-16 px-6 whitespace-nowrap">
        <div className="flex items-center gap-2">
          ...
          <time dateTime={request.timestamp} className="text-zinc-100 text-sm tracking-[-0.3008px] font-en">
            {new Date(request.timestamp).toLocaleString('en-US', DATE_FORMAT_FULL)}
          </time>
        </div>
      </td>
        ...
        <div className="relative bg-zinc-950/50 rounded-lg h-8 border border-zinc-800 w-48">
          <div className="flex items-center gap-2 h-full px-3">
            <div className="flex-1 min-w-0">
              <p className="text-zinc-300 text-sm tracking-[-0.1504px] truncate font-en">{apiKeyName}</p>
            </div>
            <button
              onClick={() => onCopy(apiKeyName)}
              className="text-zinc-500 hover:text-zinc-300 transition-colors shrink-0"
      ...
      {/* Endpoint */}
      <td className="h-16 px-6">
        <p className="text-zinc-100 text-sm tracking-[-0.3008px] truncate max-w-xs font-en">{request.endpoint}</p>
      </td>

      {/* Method */}
      <td className="h-16 px-6">
        <div className="inline-flex h-7 rounded-lg border border-white/10 items-center justify-center px-3">
          <p className="text-zinc-200 text-sm tracking-[-0.1504px] font-en">{request.method}</p>
        </div>
      </td>
      ...
      {/* Response Time */}
      <td className="h-16 px-6 whitespace-nowrap">
        <p className="text-rose-400 text-sm tracking-[-0.3008px] font-en">{request.responseTime}ms</p>
      </td>

      {/* IP Address */}
      <td className="h-16 px-6 whitespace-nowrap">
        <p className="text-zinc-400 text-sm tracking-[-0.3008px] font-en">{request.ipAddress}</p>
      </td>

      {/* Cost */}
      <td className="h-16 px-6 whitespace-nowrap">
        <p className="text-rose-400 text-sm tracking-[-0.3008px] font-en">- ${request.cost.toFixed(4)}</p>
      </td>
    </tr>
  );
}
...
export function RequestLogs({ requests, apiKeys }: RequestLogsProps) {
  ...
  const getAPIKeyName = useCallback((apiKeyId: string): string => {
  ...
  const formatDateRange = useCallback((): string => {
    ...
    if (dateRange.from && dateRange.to) {
      const fromStr = dateRange.from.toLocaleDateString('en-US', DATE_FORMAT_SHORT);
      const toStr = dateRange.to.toLocaleDateString('en-US', DATE_FORMAT_SHORT);
      return `${fromStr} - ${toStr}`;
    }
    
    if (dateRange.from) {
      return `From ${dateRange.from.toLocaleDateString('en-US', DATE_FORMAT_SHORT)}`;
    }
    
    return `Until ${dateRange.to?.toLocaleDateString('en-US', DATE_FORMAT_SHORT)}`;
  ...
  const filteredRequests = useMemo(() => {
    return requests.filter(req => {
      // Status filter
      if (filter === 'success' && (req.statusCode < 200 || req.statusCode >= 300)) return false;
      ...
      if (dateRange.from || dateRange.to) {
        ...
        if (dateRange.from && requestDateOnly < normalizeDate(dateRange.from)) return false;
        if (dateRange.to && requestDateOnly > normalizeDate(dateRange.to)) return false;
      }
    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full gap-4">
        <div className="flex flex-col gap-1">
          <h2 className={`text-zinc-100 tracking-[-0.3125px] ${fontClass}`}>{t.logs.title}</h2>
          <p className={`text-zinc-400 text-sm tracking-[-0.1504px] ${fontClass}`}>{t.logs.subtitle}</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
          ...
          <Popover open={isDatePickerOpen} onOpenChange={setIsDatePickerOpen}>
            <PopoverTrigger asChild>
              <button
                className="bg-zinc-900/50 backdrop-blur-sm border border-zinc-800/80 rounded-lg px-3 py-2 flex items-center gap-2 w-full sm:w-auto hover:border-zinc-700 transition-colors"
                aria-label="Select date range"
              >
                <CalendarIcon className="w-4 h-4 text-zinc-500 shrink-0" aria-hidden="true" />
            ...
            <PopoverContent className="w-auto p-0 bg-zinc-900 border-zinc-800" align="start" side="bottom" sideOffset={8}>
              <div className="box-border content-stretch flex flex-col gap-[16px] items-start pb-[15px] pt-[16px] px-[16px] relative w-full pr-[16px] pl-[16px]">
                <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-full">
                  <p className={`text-sm text-[#9f9fa9] ${fontClass}`}>{t.logs.selectDateRange}</p>
                ...
                <div className="content-stretch flex gap-[8px] h-[36px] items-start relative shrink-0 w-full">
                  <div 
                    className="basis-0 bg-zinc-800 grow h-[36px] min-h-px min-w-px relative rounded-[10px] shrink-0"
                    onClick={() => {
                      clearDateRange();
                  ...
                  <div 
                    className="basis-0 bg-[#1b5ba5] grow h-[36px] min-h-px min-w-px relative rounded-[10px] shrink-0"
          ...
          <div className="bg-zinc-900/50 backdrop-blur-sm border border-zinc-800/80 rounded-lg px-3 py-2 flex items-center gap-2 w-full sm:w-[152px] hover:border-zinc-700 transition-colors px-[12px] py-[0px]">
            ...
            <Select value={filter} onValueChange={(value) => setFilter(value as FilterType)}>
              <SelectTrigger 
                className={`flex-1 h-auto bg-transparent border-0 text-zinc-300 text-sm p-0 ${fontClass}`}
                aria-label="Filter by status"
              >
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all" className={fontClass}>{t.logs.all}</SelectItem>
                <SelectItem value="success" className={fontClass}>{t.logs.success}</SelectItem>
                <SelectItem value="error" className={fontClass}>{t.logs.error}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
              ...
              <thead className="sticky top-0 z-10 bg-zinc-900">
                <tr className="border-b border-zinc-800">
                  <th className="h-12 px-6 text-left">
                    <p className={`text-zinc-500 text-xs tracking-wider uppercase ${fontClass}`}>{t.logs.timestamp}</p>
                  </th>
                  <th className="h-12 px-6 text-left">
                    <p className={`text-zinc-500 text-xs tracking-wider uppercase ${fontClass}`}>{t.logs.apiKey}</p>
                  </th>
                  <th className="h-12 px-6 text-left">
                    <p className={`text-zinc-500 text-xs tracking-wider uppercase ${fontClass}`}>{t.logs.endpoint}</p>
                  </th>
                  <th className="h-12 px-6 text-left">
                    <p className={`text-zinc-500 text-xs tracking-wider uppercase ${fontClass}`}>{t.logs.method}</p>
                  </th>
                  <th className="h-12 px-6 text-left">
                    <p className={`text-zinc-500 text-xs tracking-wider uppercase ${fontClass}`}>{t.logs.status}</p>
                  </th>
                  <th className="h-12 px-6 text-left">
                    <p className={`text-zinc-500 text-xs tracking-wider uppercase ${fontClass}`}>{t.logs.response}</p>
                  </th>
                  <th className="h-12 px-6 text-left">
                    <p className={`text-zinc-500 text-xs tracking-wider uppercase ${fontClass}`}>{t.logs.ipAddress}</p>
                  </th>
                  <th className="h-12 px-6 text-left">
                    <p className={`text-zinc-500 text-xs tracking-wider uppercase ${fontClass}`}>{t.logs.cost}</p>
                  </th>
                </tr>
              </thead>

...
import { useLanguage, translateTransactionDescription } from './LanguageContext';
...
import { Wallet, Plus, Download, ArrowUpRight, ArrowDownRight, DollarSign, Eye, CheckCircle2, X, Calendar as CalendarIcon } from 'lucide-react';
...
import { generateInvoiceNumber, downloadInvoice } from '../services/invoiceService';
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between p-3 rounded-lg border border-zinc-800 hover:bg-zinc-800/30 transition-colors gap-3 sm:gap-4">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <div className={`h-8 w-8 flex-shrink-0 rounded-full flex items-center justify-center ${getIconBgColor()}`}>
          {getTransactionIcon()}
        </div>
        <div className="flex-1 min-w-0">
          <p className={`text-zinc-100 text-sm tracking-[-0.3125px] truncate font-en ${fontClass}`}>
            {translateTransactionDescription(transaction.description, language)}
          </p>
          <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
            ...
            <Badge variant="outline" className={`text-[10px] border-[rgba(255,255,255,0.1)] bg-transparent text-zinc-200 px-1.5 py-0 ${fontClass}`}>
              {transaction.type === 'top_up' ? t.wallet.topUp : t.wallet.usage}
            </Badge>
            {transaction.status === 'pending' && (
              <Badge variant="outline" className={`text-[10px] border-yellow-500/50 bg-yellow-500/10 text-yellow-400 px-1.5 py-0 ${fontClass}`}>
                {t.wallet.pending}
              </Badge>
            )}
            {transaction.status === 'completed' && transaction.type === 'top_up' && (
              <Badge variant="outline" className={`text-[10px] border-[#00c950]/50 bg-[#00c950]/10 text-[#00c950] px-1.5 py-0 ${fontClass}`}>
                {t.wallet.completed}
              </Badge>
            )}
          </div>
      ...
      <div className="flex items-center justify-between sm:justify-end gap-3 sm:gap-2">
        <div className="text-left sm:text-right">
          <p className={`text-sm tracking-[-0.4395px] font-en ${getAmountColor()}`}>
            {transaction.status === 'pending' ? '' : transaction.amount > 0 ? '+' : '-'}${Math.abs(transaction.amount).toFixed(2)}
          </p>
        </div>
        {transaction.status === 'pending' ? (
          <Button
...
export function WalletManagement({
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const allTransactions = [...userAdminTransactions, ...txns].sort((a, b) =>
  ...
  const handleConfirmTopUp = useCallback(async () => {
    ...
    try {
      const pendingTransaction = await apiServiceManager.createTopUpTransaction(userId, amount, 'KHQR', 'pending');
  ...
  const handlePaymentComplete = useCallback(async () => {
    ...
    try {
      ...
      if (pendingTransactionId) {
        const updatedTransaction = await apiServiceManager.updateTransactionStatus(pendingTransactionId, 'completed', userId);
  ...
  }, [topUpAmount, userBalance, pendingTransactionId, userId, onBalanceUpdate, loadData, onTopUpSuccess]);
  ...
  const handleSuccessClose = useCallback(() => {
    ...
    toast.success(`Successfully added $${parseFloat(topUpAmount).toFixed(2)} to your wallet`);
  ...
  const handleDownloadFromPreview = useCallback(() => {
    ...
    if (completedTransaction && completedTransaction.id === previewTransaction.id) {
      handleSuccessClose();
    }
  ...
  const handleDownloadTransactionInvoice = useCallback((transaction: Transaction) => {
    const invoiceData = {
      invoiceNumber: generateInvoiceNumber(),
      date: new Date().toLocaleDateString('en-US', {
        month: 'long',
  ...
  const formatDateRange = useCallback(() => {
    ...
    return `${t.wallet.dateRangeUntil} ${dateRange.to ? formatDate(dateRange.to) : ''}`;
  }, [dateRange, t.wallet]);
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className={`text-zinc-400 text-sm ${fontClass}`}>{t.common.loading}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className={`text-zinc-100 text-base ${fontClass}`}>{t.wallet.title}</h1>
        ...
        <Button
          ...
          className={`bg-[#1b5ba5] hover:bg-[#1b5ba5]/90 text-white h-10 py-2 px-5 ${fontClass}`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <Card className="p-5 bg-zinc-900 border border-zinc-800">
          <div className="flex items-center justify-between h-[108px]">
            <div className="flex flex-col justify-between h-full">
              <p className={`text-white text-sm ${fontClass}`}>{t.wallet.availableBalance}</p>
              <div className="flex items-baseline gap-2">
                <span className="text-white text-2xl font-en">${userBalance.toFixed(2)}</span>
                <span className="text-white text-sm font-en">USD</span>
              </div>
              <p className={`text-white text-sm ${fontClass}`}>
                {t.wallet.totalSpent}: <span className="font-en">${statistics.totalSpent.toFixed(2)}</span>
              </p>
            </div>
        ...
        <Card className="p-5 bg-zinc-900 border border-zinc-800">
          <div className="flex items-center justify-between h-[108px]">
            <div className="flex flex-col justify-between h-full">
              <p className={`text-white text-sm ${fontClass}`}>{t.wallet.totalTopUps}</p>
              <div className="flex items-baseline gap-2">
                <span className="text-white text-2xl font-en">${statistics.totalTopUps.toFixed(2)}</span>
                <span className="text-white text-sm font-en">USD</span>
              </div>
              <p className={`text-white text-sm ${fontClass}`}>
                {t.wallet.thisMonth}: <span className="font-en">${statistics.thisMonthTopUps.toFixed(2)}</span>
              </p>
            </div>
        ...
        <Tabs defaultValue="transactions" className="w-full">
          <div className="w-full border-b border-zinc-800 px-5 py-2.5">
            <TabsList className="bg-transparent h-9 p-0">
              <TabsTrigger
                value="transactions"
                className={`text-sm sm:text-base data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:text-zinc-200 text-zinc-200 px-2.5 py-1.5 ${fontClass}`}
              >
          ...
          <TabsContent value="transactions" className="px-4 sm:px-6 py-5 space-y-4 mt-0">
            {/* Filters and Search */}
            <div className="flex flex-col lg:flex-row gap-3">
              ...
              <div className="grid grid-cols-2 sm:flex gap-3 flex-wrap">
                <Select value={filterType} onValueChange={(value: FilterType) => setFilterType(value)}>
                  <SelectTrigger className={`w-full lg:w-[180px] bg-zinc-900 border-zinc-800 h-9 ${fontClass}`}>
                    <SelectValue placeholder={t.wallet.filterByType} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all" className={fontClass}>{t.wallet.allTypes}</SelectItem>
                    <SelectItem value="top_up" className={fontClass}>{t.wallet.topUp}</SelectItem>
                    <SelectItem value="usage" className={fontClass}>{t.wallet.usage}</SelectItem>
                  </SelectContent>
                </Select>
                <Select value={filterStatus} onValueChange={(value: FilterStatus) => setFilterStatus(value)}>
                  ...
                  <SelectContent>
                    <SelectItem value="all" className={fontClass}>{t.wallet.allStatus}</SelectItem>
                    <SelectItem value="pending" className={fontClass}>{t.wallet.pending}</SelectItem>
                    <SelectItem value="completed" className={fontClass}>{t.wallet.completed}</SelectItem>
                  </SelectContent>
                </Select>
                <Popover open={isDatePickerOpen} onOpenChange={setIsDatePickerOpen}>
                  <PopoverTrigger asChild>
                    <button className="bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 flex items-center gap-2 w-full lg:w-auto hover:border-zinc-700 transition-colors h-9">
                      <CalendarIcon className="w-4 h-4 text-zinc-500 shrink-0" />
                      <span className={`text-zinc-300 text-sm whitespace-nowrap ${fontClass}`}>
                        {formatDateRange()}
                      </span>
                  ...
                  <PopoverContent className="w-auto p-0 bg-zinc-900 border-zinc-800" align="start" side="bottom" sideOffset={8}>
                    <div className="box-border content-stretch flex flex-col gap-[16px] items-start pb-[15px] pt-[16px] px-[16px] relative w-full pr-[16px] pl-[16px]">
                      <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-full">
                        <div className="h-[20px] relative shrink-0 w-full">
                          <p className={`absolute font-['Open_Sans:Regular',sans-serif] font-normal leading-[20px] left-0 text-[#9f9fa9] text-[14px] text-nowrap top-[0.57px] whitespace-pre ${fontClass}`} style={{ fontVariationSettings: "'wdth' 100" }}>
                      ...
                      <div className="content-stretch flex gap-[8px] h-[36px] items-start relative shrink-0 w-full">
                        ...
                        >
                          <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-full">
                            <p className={`absolute font-['Open_Sans:Regular',sans-serif] font-normal leading-[20px] left-[60.36px] text-[14px] text-center text-nowrap text-zinc-300 top-[8.57px] translate-x-[-50%] whitespace-pre ${fontClass}`} style={{ fontVariationSettings: "'wdth' 100" }}>
                              {t.common.cancel}
                            </p>
                          </div>
                        </div>
                        ...
                        >
                          <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-full">
                            <p className={`absolute font-['Open_Sans:Regular',sans-serif] font-normal leading-[20px] left-[60.12px] text-[14px] text-center text-nowrap text-white top-[8.57px] translate-x-[-50%] whitespace-pre ${fontClass}`} style={{ fontVariationSettings: "'wdth' 100" }}>
                              {t.common.confirm}
                            </p>
                          </div>
                        </div>
                ...
                <Button variant="ghost" onClick={exportTransactions} className={`border-0 text-zinc-200 hover:bg-transparent h-9 px-4 flex-shrink-0 ${fontClass}`}>
            ...
            <div className="space-y-2">
              {filteredTransactions.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className={`text-zinc-400 ${fontClass}`}>{t.wallet.noTransactions}</p>
                </div>
              ) : (
            ...
            {filteredTransactions.length > ITEMS_PER_PAGE && (
              <div className="flex items-center justify-between gap-2 mt-6">
                <p className={`text-[#9f9fa9] text-sm ${fontClass}`}>
                  ...
                  {startIndex + 1} {t.users.to} {Math.min(endIndex, filteredTransactions.length)} {t.users.of} {filteredTransactions.length}
                  <span className="hidden sm:inline"> {t.wallet.transactions.toLowerCase()}</span>
                </p>
      ...
      <Dialog open={showTopUpDialog} onOpenChange={handleTopUpDialogClose}>
        ...
        >
          <DialogTitle className="sr-only">
            {topUpStep === 'amount' ? 'Top Up Balance' :
              topUpStep === 'scanning' ? 'Scan QR Code to Pay' :
                'Payment Successful'}
          </DialogTitle>
          <DialogDescription className="sr-only">
            {topUpStep === 'amount' ? 'Enter the amount you want to top up to your wallet' :
              topUpStep === 'scanning' ? 'Scan the QR code with your banking app to complete the payment' :
            ...
            <div className="bg-zinc-900 dark:bg-zinc-900 relative min-h-[400px] flex flex-col items-center justify-center p-5">
              <div className="flex flex-col items-center gap-6">
                ...
                <div className="text-center space-y-3">
                  <h3 className={`text-white text-3xl ${fontClass}`}>{t.topUp.paymentCompleted}</h3>
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden p-0 bg-white border-zinc-300">
          <div className="flex flex-col h-full">
            <div className="flex items-center justify-between p-6 border-b border-zinc-200 bg-white sticky top-0 z-10">
              <div>
                <DialogTitle className={`text-xl text-zinc-900 ${fontClass}`}>{t.topUp.invoicePreviewTitle}</DialogTitle>
                <DialogDescription className={`text-sm text-zinc-500 ${fontClass}`}>{t.topUp.invoicePreviewDescription}</DialogDescription>
              </div>
              <div className="flex gap-2">
                <Button
                  onClick={handleDownloadFromPreview}
                  className={`bg-[#1b5ba5] hover:bg-[#1b5ba5]/90 text-white ${fontClass}`}
                >
                  <Download className="w-4 h-4 mr-2" />
                  ...
                  <div className="bg-white border border-zinc-200 overflow-hidden shadow-lg rounded-lg w-full">
                    <div className="bg-gradient-to-br from-[#1b5ba5] to-[#0d3d6e] text-white px-7 py-5">
                      <div className="flex items-center gap-3 mb-6">
                        ...
                        <span className="text-2xl font-en">RokTenh Map API</span>
                      </div>
                      <div>
                        <p className={`text-sm opacity-90 mb-2 ${fontClass}`}>INVOICE</p>
                    ...
                    <div className="p-5">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-10 border-b-2 border-zinc-100 mb-5 pb-5">
                        <div>
                          <h3 className={`text-xs uppercase text-zinc-500 mb-3 tracking-wide ${fontClass}`}>Invoice To</h3>
                          <p className="text-zinc-900 mb-1">{DEFAULT_USER_NAME}</p>
                          <p className="text-sm text-zinc-600">{DEFAULT_USER_EMAIL}</p>
                        </div>
                        <div>
                          <h3 className={`text-xs uppercase text-zinc-500 mb-3 tracking-wide ${fontClass}`}>Invoice Details</h3>
                          <p className={`text-sm text-zinc-900 mb-1 ${fontClass}`}>
                            <span className="text-zinc-600">Date:</span> {new Date().toLocaleDateString('en-US', {
                              month: 'long',
                              day: 'numeric',
                              year: 'numeric'
                            })}
                          ...
                          <p className={`text-sm text-zinc-900 ${fontClass}`}>
                            <span className="text-zinc-600">Payment Method:</span> KHQR
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="bg-zinc-50 rounded-lg p-5 mb-5">
                        <div className="space-y-4">
                          <div className="flex justify-between py-3 border-b border-zinc-200">
                            <span className={`text-sm text-zinc-600 ${fontClass}`}>Description</span>
                            <span className={`text-sm text-zinc-900 ${fontClass}`}>Account top-up via KHQR</span>
                          </div>
                          <div className="flex justify-between py-3 border-b border-zinc-200">
                            <span className={`text-sm text-zinc-600 ${fontClass}`}>Transaction Type</span>
                            <span className="text-sm text-zinc-900">{previewTransaction.type.replace('_', ' ').toUpperCase()}</span>
                          </div>
                          <div className="flex justify-between py-3">
                            <span className={`text-sm text-zinc-600 ${fontClass}`}>Processing Fee</span>
                      ...
                      <div className="bg-[#1b5ba5] text-white rounded-lg p-5">
                        <div className="flex justify-between items-center">
                          <div>
                            <p className={`text-base opacity-90 mb-3 ${fontClass}`}>Total Amount</p>
                            <div className={`inline-block px-4 py-1.5 bg-green-500 rounded-full text-xs uppercase tracking-wider ${fontClass}`}>
                              PAID
                            </div>
                          </div>
                          <p className="text-4xl tracking-tight">${Math.abs(previewTransaction.amount).toFixed(2)}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-zinc-50 border-t border-zinc-200 p-5">
                      <div className="text-center">
                        <h4 className="text-sm text-zinc-900 mb-2 font-en">RokTenh Map API</h4>
                        <p className={`text-xs text-zinc-600 mb-1 ${fontClass}`}>Map Service Platform</p>
                        <p className={`text-xs text-zinc-600 mb-4 ${fontClass}`}>Providing geocoding, routing, and place search services</p>
                        <div className="pt-4 border-t border-zinc-200">
                          <p className={`text-xs text-zinc-500 mb-1 ${fontClass}`}>This is an automated invoice. No signature required.</p>
                          <p className={`text-xs text-zinc-400 ${fontClass}`}>© {new Date().getFullYear()} RokTenh Map API. All rights reserved.</p>
                        </div>
                      </div>
                    </div>
                  </div>

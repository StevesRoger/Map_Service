...
import { Eye, EyeOff, Upload, Camera, User, Mail, Phone, MapPin, Lock, Shield, Bell, Palette, Globe, Save, Loader2 } from 'lucide-react';
...
export function Settings() {
  ...
  const [fullName, setFullName] = useState(user?.name || 'Narith');
  const [email, setEmail] = useState(user?.email || 'admin@roktenh.io');
  const [phone, setPhone] = useState('+1 (555) 123-4567');
  const [address, setAddress] = useState('RokTenh Technologies');
  const [profilePhoto, setProfilePhoto] = useState<string | null>(null);

  // Track original values for change detection
  const [originalValues, setOriginalValues] = useState({
    fullName: user?.name || 'Narith',
    email: user?.email || 'admin@roktenh.io',
    phone: '+1 (555) 123-4567',
    address: 'RokTenh Technologies'
  });

  // Load profile photo and saved data from localStorage on mount
  useEffect(() => {
    ...
    const savedFullName = localStorage.getItem('roktenh_fullname');
    const savedEmail = localStorage.getItem('roktenh_email');
    const savedPhone = localStorage.getItem('roktenh_phone');
    const savedAddress = localStorage.getItem('roktenh_address');

    if (savedFullName) {
      setFullName(savedFullName);
      setOriginalValues(prev => ({ ...prev, fullName: savedFullName }));
    }
    if (savedEmail) {
      setEmail(savedEmail);
      setOriginalValues(prev => ({ ...prev, email: savedEmail }));
    }
    if (savedPhone) {
      setPhone(savedPhone);
  ...
  const sendOTP = (emailAddress: string) => {
    ...
    toast.success(`${language === 'km' ? 'លេខកូដផ្ទៀងផ្ទាត់ត្រូវបានផ្ញើទៅ' : 'Verification code sent to'} ${emailAddress}`, {
      description: `${language === 'km' ? 'លេខកូដសាកល្បង' : 'Demo OTP'}: ${otp} (${language === 'km' ? 'ប្រើលេខកូដនេះដើម្បីផ្ទៀងផ្ទាត់' : 'Use this code to verify'})`
    });
  };
  ...
  const handleResendOTP = () => {
    if (otpCountdown > 0) {
      toast.error(language === 'km' ? 'សូមរង់ចាំមុនពេលស្នើសុំលេខកូដថ្មី' : 'Please wait before requesting a new OTP');
      return;
    }
    setOtpCountdown(300);
    setOtpCode('');
  ...
  const handleVerifyOTP = () => {
    if (otpCode.length !== 6) {
      toast.error(language === 'km' ? 'សូមបញ្ចូលលេខកូដ 6 ខ្ទង់' : 'Please enter a 6-digit OTP code');
      return;
    }
    ...
    input.onchange = (e: any) => {
      ...
      if (file) {
        ...
        if (file.size > 5 * 1024 * 1024) {
          toast.error(language === 'km' ? 'ទំហំឯកសារត្រូវតែតិចជាង 5MB' : 'File size must be less than 5MB');
          return;
        }

        // Show loading state
        setIsUploadingPhoto(true);
        toast.info(language === 'km' ? 'កំពុងផ្ទុករូបថត...' : 'Uploading photo...');
        ...
        reader.onload = (event) => {
          ...
          setTimeout(() => {
            ...
            toast.success(language === 'km' ? 'រូបថតត្រូវបានផ្ទុកឡើង និងរក្សាទុកបានជោគជ័យ!' : 'Photo uploaded and saved successfully!');
          }, 500);
        };
        reader.onerror = () => {
          setIsUploadingPhoto(false);
          toast.error(language === 'km' ? 'បរាជ័យក្នុងការផ្ទុករូបថត' : 'Failed to upload photo');
        };
  ...
  const handleLogout = () => {
    logout();
    toast.success(language === 'km' ? 'ចាកចេញបានជោគជ័យ' : 'Logged out successfully');
  };
  ...
  const handleSaveChanges = () => {
    ...
    if (emailChanged) {
      ...
      if (!email || !emailRegex.test(email)) {
        toast.error(language === 'km' ? 'សូមបញ្ចូលអាសយដ្ឋានអ៊ីមែលត្រឹមត្រូវ' : 'Please enter a valid email address');
        return;
      }
    ...
    if (oldPassword || newPassword || confirmPassword) {
      if (!oldPassword || !newPassword || !confirmPassword) {
        toast.error(language === 'km' ? 'សូមបំពេញក្របខណ្ឌពាក្យសម្ងាត់ទាំងអស់ ឬទុកវាឲ្យទទេ' : 'Please fill in all password fields or leave them empty');
        return;
      }

      if (newPassword !== confirmPassword) {
        toast.error(language === 'km' ? 'ពាក្យសម្ងាត់ថ្មីមិនត្រូវគ្នា' : 'New passwords do not match');
        return;
      }

      if (newPassword.length < 8) {
        toast.error(language === 'km' ? 'ពាក្យសម្ងាត់ត្រូវតែមានយ៉ាងហោចណាស់ 8 តួអក្សរ' : 'Password must be at least 8 characters');
        return;
      }
      ...
      toast.success(language === 'km' ? 'ការកំណត់ទាំងអស់ត្រូវបានរក្សាទុកបានជោគជ័យ!' : 'All settings saved successfully!');
    } else {
      // Only account info was updated
      toast.success(language === 'km' ? 'ព័ត៌មានគណនីត្រូវបានរក្សាទុកបានជោគជ័យ!' : 'Account information saved successfully!');
    }
  ...
  const translations = {
    ...
    manageAccountSettings: language === 'km' ? 'គ្រប់គ្រងការកំណត់គណនីរបស់អ្នកនៅទីនេះ' : 'Manage your account settings here',
    uploadPhoto: language === 'km' ? 'ផ្ទុករូបថត' : 'Upload Photo',
    uploading: language === 'km' ? 'កំពុងផ្ទុក...' : 'Uploading...',
    photoRestriction: language === 'km' ? 'អនុញ្ញាតតែរូបថត JPG, JPEG, ឬ PNG ប៉ុណ្ណោះ។ រក្សាទុកដោយស្វ័យប្រវត្តិនៅពេលជ្រើសរើស។' : 'Only JPG, JPEG, or PNG photos are allowed. Auto-saves when selected.',
    accountInformation: language === 'km' ? 'ព័ត៌មានគណនី' : 'Account Information',
    ...
    createNewPassword: language === 'km' ? 'បង្កើតពាក្យសម្ងាត់ថ្មី' : 'Create New Password',
    confirmNewPassword: language === 'km' ? 'បញ្ជាក់ពាក្យសម្ងាត់ថ្មី' : 'Confirm New Password',
    verifyEmailAddress: language === 'km' ? 'ផ្ទៀងផ្ទាត់អាសយដ្ឋានអ៊ីមែល' : 'Verify Email Address',
    verificationCodeSent: language === 'km' ? 'យើងបានផ្ញើលេខកូដផ្ទៀងផ្ទាត់ 6 ខ្ទង់ទៅ' : 'We\'ve sent a 6-digit verification code to',
    enterSixDigitCode: language === 'km' ? 'បញ្ចូលលេខកូដ 6 ខ្ទង់' : 'Enter 6-digit code',
    demoModeCode: language === 'km' ? 'ម៉ូដសាកល្បង: ប្រើលេខកូដ' : 'Demo mode: Use code',
    toVerify: language === 'km' ? 'ដើម្បីផ្ទៀងផ្ទាត់' : 'to verify',
    codeExpiresIn: language === 'km' ? 'លេខកូដផុតកំណត់ក្នុងរយៈពេល' : 'Code expires in',
    ...
    <div className="flex flex-col gap-6 w-full py-[0px] p-[0px]">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex flex-col gap-1">
          <h2 className={`text-zinc-100 tracking-[-0.3125px] ${fontClass}`}>{translations.accountSettings}</h2>
          <p className={`text-zinc-400 tracking-[-0.1504px] text-xs sm:text-sm ${fontClass}`}>{translations.manageAccountSettings}</p>
        </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-[14px] p-[20.8px] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4 sm:gap-6">
          ...
          <div className="flex flex-col gap-3">
            ...
            <p className={`text-zinc-400 text-[13px] max-w-[631px] ${fontClass}`}>
      ...
      <div className="flex flex-col lg:flex-row gap-5">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-[14px] p-[20.8px] w-full lg:w-[454.4px] flex flex-col gap-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <h3 className={`text-zinc-100 text-base ${fontClass}`}>{translations.accountInformation}</h3>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              <label className={`text-zinc-300 text-sm ${fontClass}`}>{t.settings.fullName}</label>
              <input
                type="text"
                value={fullName}
            ...
            {/* Email Address */}
            <div className="flex flex-col gap-2">
              <label className={`text-zinc-300 text-sm ${fontClass}`}>{t.settings.email}</label>
              <input
                type="email"
                value={email}
            ...
            {/* Phone Number */}
            <div className="flex flex-col gap-2">
              <label className={`text-zinc-300 text-sm ${fontClass}`}>{translations.phoneNumber}</label>
              <input
                type="tel"
                value={phone}
            ...
            {/* Address Name */}
            <div className="flex flex-col gap-2">
              <label className={`text-zinc-300 text-sm ${fontClass}`}>{translations.addressName}</label>
              <input
                type="text"
                value={address}
        ...
        <div className="flex-1 h-fit bg-zinc-900 border border-zinc-800 rounded-[14px] p-[20.8px] flex flex-col gap-5">
          ...
          <div className="pb-4 border-b border-zinc-800/50">
            <p className={`text-white text-sm ${fontClass}`}>{t.settings.changePassword}</p>
          </div>

          {/* Old Password */}
          <div className="flex flex-col gap-1.5">
            <label className={`text-zinc-400 text-xs ${fontClass}`}>{translations.oldPassword}*</label>
          ...
          <div className="flex flex-col sm:flex-row gap-5">
            ...
            <div className="flex-1 flex flex-col gap-1.5">
              <label className={`text-zinc-400 text-xs ${fontClass}`}>{translations.createNewPassword}*</label>
              <div className="relative">
                <input
            ...
            <div className="flex-1 flex flex-col gap-1.5">
              <label className={`text-zinc-400 text-xs ${fontClass}`}>{translations.confirmNewPassword}*</label>
              <div className="relative">
                <input
      ...
      <Dialog open={showOtpDialog} onOpenChange={setShowOtpDialog}>
        <DialogContent className="bg-zinc-900 border-zinc-800 max-w-md p-0 gap-0">
          ...
          <div className="p-6 pb-4 border-b border-zinc-800/50">
            ...
            <DialogDescription className={`text-zinc-400 text-sm ml-[52px] ${fontClass}`}>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-center gap-3 p-3 rounded-lg bg-zinc-800/50 border border-zinc-700/50">
              ...
              <span className="text-zinc-200 text-sm font-en flex-1">{pendingEmail}</span>
              <button
                onClick={() => {
                  setShowOtpDialog(false);
                  setOtpCode('');
            ...
            <div className="space-y-3">
              ...
              {/* Demo Code Info */}
              <div className={`text-center p-3 rounded-lg bg-blue-500/5 border border-blue-500/20 ${fontClass}`}>
                <p className="text-xs text-zinc-400">
                  {translations.demoModeCode}{' '}
                  <span className="text-blue-400 font-mono font-en text-sm">123456</span>
                </p>
              </div>
            </div>
                ...
                {otpCountdown > 0 ? (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-zinc-400">
                      ...
                      <span className="text-zinc-200 font-en font-mono">{formatTime(otpCountdown)}</span>
                    </span>
                  </div>
                ) : (
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-red-500"></div>
                    <span className="text-red-400">{translations.codeExpired}</span>
                  </div>
                )}
              </div>
              <button
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button
                ...
                disabled={otpCode.length !== 6 || isVerifying || otpCountdown === 0}

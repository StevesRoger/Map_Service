...
import { Check, DollarSign, Zap, Send, Phone, MapPin, Navigation, Search, Map } from 'lucide-react';
...
export function PricingPlans({ onNavigateToWallet }: PricingPlansProps) {
  ...
  const calculateServiceCost = (serviceRequests: number, pricePerRequest: number) => {
    ...
    <div className="mx-auto space-y-10 px-[0px] max-w-[1920px] w-full py-[0px] p-[0px]">
      ...
      <div className="text-center space-y-3 mt-[0px] mr-[0px] mb-[20px] ml-[0px]">
        <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-500/10 dark:bg-blue-500/20 rounded-full mb-2">
          ...
          <span className={`text-sm text-blue-600 dark:text-blue-400 font-en`}>{t.pricing.lowerThanGoogle}</span>
        </div>
        <h2 className={`text-zinc-900 dark:text-zinc-100 text-[20px] ${fontClass}`}>{t.pricing.title}</h2>
        <p className={`text-zinc-600 dark:text-zinc-400 max-w-2xl mx-auto my-[0px] text-[16px] ${fontClass}`}>
          {t.pricing.subtitle}
        </p>
      </div>
      ...
      <Card className="bg-gradient-to-r from-green-500/10 to-blue-500/10 border-green-500/20 dark:border-green-500/20 mt-[0px] mr-[0px] mb-[10px] ml-[0px] px-[20px] py-[21px]">
        <div className="flex items-center justify-center gap-8 flex-wrap">
          <div className="text-center">
            <p className={`text-sm text-zinc-600 dark:text-zinc-400 mb-1 font-en`}>Google Maps API</p>
            <p className="text-2xl text-zinc-400 line-through font-en">$5.00</p>
            <p className={`text-xs text-zinc-500 ${fontClass}`}>{t.pricing.perKGeocoding}</p>
          </div>
          <div className="text-4xl text-green-500">→</div>
          <div className="text-center">
            <p className={`text-sm text-zinc-600 dark:text-zinc-400 mb-1 font-en`}>RokTenh Map API</p>
            <p className="text-2xl text-green-500 dark:text-green-400 font-en">$2.50</p>
            <p className={`text-xs text-green-600 dark:text-green-400 font-en`}>{t.pricing.save50Percent}</p>
          </div>
        </div>
      </Card>
      ...
      <div className="space-y-6">
        <div>
          <h3 className={`text-zinc-900 dark:text-zinc-100 mb-2 ${fontClass}`}>{t.pricing.apiServicesPricing}</h3>
          <p className={`text-sm text-zinc-600 dark:text-zinc-400 ${fontClass}`}>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {apiServices.map((service, index) => {
            ...
            return (
              <Card key={service.nameKey} className="p-6 bg-white dark:bg-card border-zinc-200 dark:border-zinc-800">
                <div className="space-y-4">
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h4 className={`text-zinc-900 dark:text-zinc-100 font-en`}>{t.pricing[service.nameKey]}</h4>
                        <p className={`text-sm text-zinc-600 dark:text-zinc-400 mt-1 ${fontClass}`}>
                  ...
                  <div className="flex items-baseline gap-2 p-3 bg-zinc-50 dark:bg-zinc-900/50 rounded-lg">
                    ...
                    <span className={`text-sm text-zinc-600 dark:text-zinc-400 ${fontClass}`}>
                      {t.pricing.perThousandRequests}
                    </span>
                  </div>

                  <div className="space-y-2">
                    <Label className={`text-xs text-zinc-600 dark:text-zinc-400 ${fontClass}`}>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setRequests({ ...requests, [service.serviceKey]: value });
                      }}
                  ...
                  <div className="pt-3 border-t border-zinc-200 dark:border-zinc-800">
                    <div className="flex justify-between items-center mb-2">
                      <span className={`text-sm text-zinc-600 dark:text-zinc-400 ${fontClass}`}>{t.pricing.estimatedCost}</span>
                    ...
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-zinc-500 dark:text-zinc-500 font-en">
                        Google Maps API: ${(requests[service.serviceKey] || 0 * service.googlePrice / 1000).toFixed(2)}/mo
                      </span>
        ...
        <div className="relative">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <h3 className={`text-white text-xl ${fontClass}`}>{t.pricing.totalMonthlyCost}</h3>
                <Badge className={`bg-white/20 text-white border-white/30 text-xs ${fontClass}`}>
                  {t.pricing.estimated}
                </Badge>
              </div>
              <p className={`text-purple-100 text-sm ${fontClass}`}>
                {t.pricing.basedOnRequests.replace('{count}', formatNumber(totalRequests, { locale: 'en', unitSpace: language === 'km' }))}
              </p>
            </div>
            ...
            <div className="text-center md:text-right">
              ...
              <p className="text-sm text-purple-100 mt-2 font-en">
                {t.pricing.averagePerRequest.replace('{cost}', totalRequests > 0 ? (totalCost / totalRequests * 1000).toFixed(3) : '0.000')}
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 pt-6 border-t border-white/20">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <p className={`text-purple-100 mb-1 ${fontClass}`}>{t.pricing.rokTenhTotal}</p>
                <p className="text-white text-lg font-en">${totalCost.toFixed(2)}</p>
              </div>
              <div>
                <p className={`text-purple-100 mb-1 ${fontClass}`}>{t.pricing.googleEquivalent}</p>
                <p className="text-white/60 text-lg line-through font-en">${(totalCost * 2).toFixed(2)}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <Card className="p-8 bg-zinc-900/30 dark:bg-zinc-900/30 border-zinc-800 dark:border-zinc-800">
        <div className="max-w-3xl mx-auto text-center space-y-6">
          <div className="space-y-2">
            <h3 className={`text-zinc-100 dark:text-zinc-100 ${fontClass}`}>{t.pricing.needEnterprise}</h3>
            <p className={`text-sm text-zinc-400 dark:text-zinc-400 ${fontClass}`}>

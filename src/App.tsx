...
const APIKeyManager = lazy(() =>
  import("./components/APIKeyManager").then((m) => ({
    default: m.APIKeyManager,
  }))
);
const RequestLogs = lazy(() =>
  import("./components/RequestLogs").then((m) => ({ default: m.RequestLogs }))
);
...
const PricingPlans = lazy(() =>
  import("./components/PricingPlans").then((m) => ({ default: m.PricingPlans }))
);
const PricingManagement = lazy(() =>
  import("./components/PricingManagement").then((m) => ({
    default: m.PricingManagement,
  }))
);
const WalletManagement = lazy(() =>
  import("./components/WalletManagement").then((m) => ({
...
const UserManagement = lazy(() =>
  import("./components/UserManagement").then((m) => ({
...
function AppContent() {
  ...
  const [isZeroBalanceAlertDismissed, setIsZeroBalanceAlertDismissed] =
    useState(false);
  const [isLowBalanceAlertDismissed, setIsLowBalanceAlertDismissed] =
    useState(false);
  const [adminTransactions, setAdminTransactions] = useState<Transaction[]>([]);
  const [notifications, setNotifications] = useState<Notification[]>([]);
  const [hasShownWelcomeNotification, setHasShownWelcomeNotification] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const updatedZero = NotificationService.notifyBalanceZero(
      notifications,
      currentUserBalance
    );
    ...
    const updatedLow = NotificationService.notifyLowBalance(
      notifications,
      currentUserBalance
    );
  ...
  useEffect(() => {
    if (isAuthenticated && !isNewUser && user && !hasShownWelcomeNotification) {
      const hasWelcomeNotif = notifications.some((n) =>
        n.title.includes("Welcome")
      );
      if (!hasWelcomeNotif) {
        const updated = NotificationService.notifyWelcome(
          notifications,
          user.name || "there"
        );
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const [keysData, requestsData, statsData, endpointsData] =
        await Promise.all([
  ...
  const handleCreateKey = useCallback(async (data: { name: string }) => {
    try {
      ...
      setNotifications((prev) =>
        NotificationService.notifyAPIKeyCreated(prev, data.name)
      );
    } catch (error) {
  ...
  const handleDeleteKey = useCallback(
    async (id: string) => {
      try {
        ...
        if (keyToDelete) {
          setNotifications((prev) =>
            NotificationService.notifyAPIKeyDeleted(prev, keyToDelete.name)
          );
        }
      } catch (error) {
  ...
  const handleToggleStatus = useCallback(
    async (id: string, status: APIKey["status"]) => {
      try {
        const keyToUpdate = apiKeys.find((key) => key.id === id);
        const updatedKey = await apiServiceManager.updateAPIKeyStatus(
          id,
          status
        );
        setApiKeys((prev) =>
        ...
        if (keyToUpdate) {
          setNotifications((prev) =>
            NotificationService.notifyAPIKeyStatusChanged(
              prev,
              keyToUpdate.name,
              status
            )
  ...
  const handleAdjustBalance = (userId: string, amount: number) => {
    ...
    if (amount > 0 && adminUser.totalBalance < amount) {
      toast.error(
        `Insufficient admin wallet balance. Available: $${adminUser.totalBalance.toFixed(
          2
        )}`
      );
      return;
    ...
    if (amount < 0 && targetUser.totalBalance + amount < 0) {
      toast.error(
        `Cannot deduct more than user's current balance of $${targetUser.totalBalance.toFixed(
          2
        )}`
      );
      return;
    ...
    const adminTransaction: Transaction = {
      ...
      description: `Transfer to ${targetUser.name}: ${
    ...
    if (amount > 0) {
      toast.success(
        `Successfully transferred $${amount.toFixed(2)} from admin wallet to ${
          targetUser.name
        }`
      );
    } else {
      toast.success(
        `Successfully deducted $${Math.abs(amount).toFixed(2)} from ${
          targetUser.name
        } (returned to admin wallet)`
      );
    }
  };
  ...
  const navItems = useMemo(
    () => [
      ...
      ...(user?.role === "admin"
        ? [{ id: "users", label: t.nav.userManagement, icon: UserCog }]
        : []),
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <div>
              ...
              <p className="text-[10px] sm:text-xs text-zinc-400 font-en">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button
                  ...
                  aria-label={
                    language === "en" ? "Switch language" : "ប្ដូរភាសា"
                  }
                >
                    ...
                    >
                      <div className="flex items-center gap-3 w-full">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span
                            className={`text-xs text-zinc-500 ${
                              option.code === "km" ? "font-kh" : "font-en"
                            }`}
                          >
                            {option.nativeLabel}
                          </span>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <div className="flex items-center gap-2 px-2 sm:px-3 py-1.5 hover:bg-zinc-800 rounded-lg cursor-pointer transition-colors">
                  ...
                  <span className="text-sm text-white hidden sm:inline font-en">
      ...
      <div className="flex-1 flex overflow-hidden">
                ...
                const button = (
                  <button
                    ...
                    className={`
                      ...
                      } px-3 py-2.5 md:py-2 lg:py-2.5 rounded-lg 
                  ...
                  >
                    ...
                    {!isCollapsed && (
                      <span
                        className={`text-sm font-bold font-normal ${fontClass}`}
                      >
                        {item.id === "wallet" && user?.role === "admin"
                          ? "Credit Wallet"
                          : item.label}
                      </span>
                    )}
              ...
              {(!isSidebarCollapsed || isMobileMenuOpen) && (
                <div className="p-3 bg-zinc-800/50 rounded-lg border border-zinc-700">
                  ...
                  <div className="text-2xl text-white mb-3 text-[20px]">
                    ...
                    <span className="text-sm font-en">USD</span>
                  </div>
                  <Button
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    ...
                    className={`hidden lg:flex w-full items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100 transition-colors ${
                  ...
                  >
                    ...
                    <span className="text-xs">
                      {isSidebarCollapsed ? t.common.open : t.common.close}
                    </span>
                  </button>
                </TooltipTrigger>
                <TooltipContent side="right">
        ...
        <main className="flex-1 overflow-y-auto bg-[rgb(0,0,0)] flex flex-col">
          <div className="max-w-7xl flex-1 w-full mx-auto px-4 sm:px-6 lg:px-8 py-[20px] p-[20px] max-w-[1920px]">
              ...
              return (
                <>
                  ...
                  {currentUserBalance === 0 && !isZeroBalanceAlertDismissed && (
                    <div className="mb-6 p-4 bg-red-100 dark:bg-red-950/30 rounded-lg border border-red-300 dark:border-red-900/50">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            className={`text-sm text-red-800 dark:text-red-300 ${fontClass}`}
                          >
                            {t.alerts.zeroBalance}
                          </p>
                          <p
                            className={`text-xs text-red-700 dark:text-red-400/80 mt-1 ${fontClass}`}
                          >
                  ...
                  {currentUserBalance > 0 &&
                    currentUserBalance < 1.99 &&
                    !isLowBalanceAlertDismissed && (
                      <div className="mb-6 p-3 sm:p-4 bg-yellow-100 dark:bg-yellow-950/30 rounded-lg border border-yellow-300 dark:border-yellow-900/50">
                        ...
                        <div className="flex sm:hidden flex-col">
                          <div className="flex items-start gap-3 mb-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <p
                                className={`text-sm text-yellow-800 dark:text-yellow-300 ${fontClass}`}
                              >
                                {t.alerts.lowBalance}
                              </p>
                              <p
                                className={`text-xs text-yellow-700 dark:text-yellow-400/80 mt-1 ${fontClass}`}
                              >
                                {t.alerts.lowBalanceDesc}
                              </p>
                              <div className="flex items-center gap-1.5 mt-2">
                                <span
                                  className={`text-xs text-yellow-600 dark:text-yellow-500 ${fontClass}`}
                                >
                                  {t.alerts.remaining}:
                                </span>
                            ...
                            <Button
                              onClick={() =>
                                setIsLowBalanceAlertDismissed(true)
                              }
                              size="sm"
                              variant="ghost"
                        ...
                        <div className="hidden sm:flex items-start sm:items-center gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            >
                              ...
                              {t.alerts.remaining.toLowerCase()}
                            </p>
                            <p
                              className={`text-xs text-yellow-700 dark:text-yellow-400/80 mt-1 ${fontClass}`}
                            >
            ...
            {activeTab === "dashboard" && (
              <Suspense
                fallback={<LoadingFallback text="Loading Dashboard..." />}
              >
                <AdminDashboard
                  stats={stats}
              ...
              <Suspense fallback={<LoadingFallback text="Loading Wallet..." />}>
                <WalletManagement
                  ...
                  onTopUpSuccess={(amount, newBalance) => {
                    ...
                    const updated = NotificationService.notifyTopUpSuccess(
                      notifications,
                      amount,
                      newBalance
                    );
            ...
            {activeTab === "logs" && (
              <Suspense
                fallback={<LoadingFallback text="Loading Request Logs..." />}
              >
            ...
            {activeTab === "docs" && (
              <Suspense
                fallback={<LoadingFallback text="Loading Documentation..." />}
              >
            ...
            {activeTab === "pricing-management" && user?.role === "admin" && (
              <Suspense
                fallback={
                  <LoadingFallback text="Loading Pricing Management..." />
                }
              >
                <PricingManagement />
              </Suspense>
            ...
            {activeTab === "settings" && (
              <Suspense
                fallback={<LoadingFallback text="Loading Settings..." />}
              >
                <SettingsPage />
              </Suspense>
            )}
          ...
          <footer className="bg-zinc-900 dark:bg-zinc-900 border-t border-zinc-800 dark:border-zinc-800 px-4 sm:px-6 lg:px-8 py-3 rounded-lg mx-4 sm:mx-6 lg:mx-8 mb-4">
            <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
              <p className="text-xs text-zinc-500">
                © 2014-2025 {t.footer.allRightsReserved}
              </p>
